...
def run_ionosphere_data_experiment(run_type, regularization_parameter, sgd_learning_rate=1e-6, sgd_num_epochs=2000000,
    ...
    formulation of the infinite-width neural network (NN) training problem for the Ionosphere dataset. As baseline
    approaches, it runs the SGD solution of the same training problem.

    Vincent G Sigillito, Simon P Wing, Larrie V Hutton, and Kile B Baker. Classification of radar returns from the
    ionosphere using neural networks. Johns Hopkins APL Technical Digest, 10(3):262–266, 1989.
    ...
    @return: a nested dictionary with the following keys depending on run_type:
        'sgd_results': a dictionary with the following keys:
                ...
                'obj_type': the type of objective loss
    ...
    """
    # Size parameters for the Ionosphere dataset
    num_hidden_neurons = [300]  # number of hidden neurons
    d = 34  # input dimension
    c = 3  # output dimension
    n = 175  # number of datapoints
    ...
    df = pd.read_csv(os.path.join('data', 'ionosphere.data'))
    ...
    # Generate CVX solution for the Ionosphere dataset
    if run_type == 'CVX':
